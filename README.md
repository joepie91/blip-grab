...
More information about the archiving project can be found on the ArchiveTeam wiki: [blip](http://archiveteam.org/index.php?title=Blip.tv)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "blip" project in the Warrior interface.
...
### For Debian/Ubuntu:
    ...
    pip install seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/blip-grab.git; cd blip-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/blip-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, blip-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/blip-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
Have an issue not listed here? Join us on IRC and ask! We can be found at irc.efnet.org #blooper.tv.
